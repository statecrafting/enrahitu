...
None claimed yet (draft). With the code, this spec establishes
`core/ledger/postgres.ts` (the `PostgresDriver`) and amends the driver
selection logic in `core/ledger/ledger.ts`; `core/` as a directory stays
owned by spec 003.
